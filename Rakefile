...
task :generate_emojis do
  Rake::Task["openmoji"].invoke("openmoji", "https://github.com/hfg-gmuend/openmoji/releases/latest/download/openmoji-72x72-color.zip")
end

desc "Download and process OpenMoji emoji set"
task :openmoji, [:name, :url] do |_, args|
  raise "Missing arguments" unless args[:name] && args[:url]
